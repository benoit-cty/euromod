...
On a machine with an NVIDIA card, the ingest package's CUDA wheels live in a second environment (UV_PROJECT_ENVIRONMENT=.venv-cuda uv sync --extra embeddings-cuda; CUDA and CPU torch are conflicting extras). query_encoder.embedding_process() is the single place that decides: if that environment exists it runs uv with --extra embeddings-cuda and UV_PROJECT_ENVIRONMENT pointed at it, otherwise nothing changes. scout.py's incremental embeddings build goes through the same helper — and skips the OpenVINO model path there, since the IR only loads under the OpenVINO backend. Measured on the GTX 1080 Ti box: ~18.7 chunks/s against ~0.93 on the CPU.
