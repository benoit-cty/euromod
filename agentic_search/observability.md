...
- **Stay inside the reproducibility backbone we already committed to.** Run manifests (pinned model/prompt/config) live in GitLab; the ingested corpus is pinned via `fetch_runs.frozen_label` ([11_database-model.md](../RAG/11_database-model.md) §7). Observability tooling should sit *on top of* that record, not replace it.
...
- **Structured extraction pass** (rate tables/schedules, [02_activity2_rag-architecture.md](../RAG/02_activity2_rag-architecture.md) §1.4): traced like any other generation step — it's an LLM call whose errors matter for the KPI report.
