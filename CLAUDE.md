...
- **`Nomotheca-RAG/ingest`** (`nomotheca-ingest`) — Activity 2. Archive-first legislation ingester. Fetches → snapshots → parses → chunks → loads the `legislation` DB, then builds BGE-M3 vector embeddings and translates non-English texts to English. Country-agnostic `core/` (~80%) + per-country adapters under `countries/<cc>/` (fetcher, parser, resolver). Adding a country is an adapter, not a rewrite; adapters are resolved via `countries/registry.py::get_adapter`. Library-first: the CLI and a future cache-miss workflow both call the same `ingest_citation` / `ingest_instrument` functions. Two source quirks are handled in the adapters: TAR never consolidates a never-amended act, so the LT adapter parses the Dokumentas row's own text as a `<id>/orig` version when the consolidation index is empty (the annual social-fund budget laws); and BWB publishes some statutory formulas as PNG images (Participatiewet art. 22a), which the NL parser renders as `[formule als afbeelding: …]` — NL has no `reparse` path, refetch the act instead. Archive-first pays off through `cli reparse <CC> --url-like …`: after a parser change it replays the stored snapshot bytes through parser and loader without refetching (the FR parser now keeps Legifrance's NOTA application note — «applicables à l'imposition des revenus de l'année 2025» — as a trailing `NOTA :` paragraph, which is where a consolidated article says which income year it applies to); changed texts leave stale vectors for `embeddings build` to refresh. See [Nomotheca-RAG/12_ingestion-architecture.md](Nomotheca-RAG/12_ingestion-architecture.md) for the shared/adapter boundary.
...
cd Nomoscope-agentic-workflow/ui && npm test                             # search-hit highlighting (node --test)
...
Models everywhere are **provider-prefixed strings**: `anthropic/…`, `openai/…`, `azure_openai/…`, `openrouter/…`, `together/…`, plus `mock/…` for key-less demos and deterministic tests. **Nothing in the pipeline knows the provider** — endpoint flexibility is a hard contractual requirement (Activity 5 compares multiple LLMs). The shared connection lives in [Nomoscope-agentic-workflow/pipeline/src/nomoscope_workflow/llm.py](Nomoscope-agentic-workflow/pipeline/src/nomoscope_workflow/llm.py); `nomotheca_ingest` and `nomokrisis_eval` both reuse it. For `azure_openai/<name>` the name after the slash **is the Azure deployment**; `AZURE_OPENAI_DEPLOYMENT` is only a default for a bare `azure_openai`, never an override — it used to be one, and every `azure_openai/*` run between 5 Aug and 8 Sep 2026 (terra, nano-2, Mistral, DeepSeek, sol…) silently ran on the `gpt-5.6-luna` deployment, judge included. `manifest.json` now records `resolved_model` / `resolved_critique_model`; before comparing two runs as two models, check those differ and that Phoenix's LLM spans (`attributes->'llm'->>'model_name'`) agree.
...
- **Every instrument carries a source-trust class, and evidence retrieval excludes by class.** `instruments.source_trust_class` is `evidence` (legislation and statute-level acts), `guidance` (circulars, doctrine — citable, ranked equal, but shown as guidance on every proposal and counted in the eval) or `context` (never citable). Retrieval filters `source_trust_class <> 'context'` — never on an `instrument_type` string — and the class travels on every hit and citation. For a contributed document it is derived from the kind the reviewer states and never stated directly; "other" is guidance, so an unnamed document cannot be granted statute-level trust by accident. See [docs/adr/0001-guidance-is-citable-evidence-with-a-visible-class.md](docs/adr/0001-guidance-is-citable-evidence-with-a-visible-class.md).
- **Country Reports are context, never evidence.** EUROMOD Country Reports are ingested with `instrument_type='country_report'` and `source_trust_class='context'`; Nomoscope's evidence retrieval SQL excludes them by that class (see the rule above), and `retrieval.country_report_search()` is the only sanctioned way to read them (query framing, UI display). Never let `propose`/`critique` cite a CR chunk — the CR describes the model, so citing it as support is circular.
...
- **Regional acts are child jurisdictions, and retrieval is scoped by the parameter's region** (ADR 0003). Spain's autonomous-community acts are filed under `ES-AR`, `ES-CN`, … (`jurisdictions.parent_id = ES`, seeded from one table in `Nomotheca-RAG/ingest/src/nomotheca_ingest/countries/es/regions.py`; existing databases run `db/migrations/0002_es_autonomous_communities.sql`). Evidence retrieval filters on a jurisdiction *scope* — `[country]` for a national parameter, `[country, region]` for a regional one, the region read off the constant's NUTS-2 digits by `regions.region_key` (`$bsarg_rg24_*` → `ES24` → `ES-AR`) — never on the whole tree: Aragón's parameter must not be "supported" by a verbatim quote from Asturias's decree, a false positive the extract check cannot catch. Regional citations are qualified by region (`Ley 3/2021 Asturias Artículo 14`; Aragón has a Ley 3/2021 too), and `EsResolver` refuses a bare regional number. BOE consolidates the communities' framework laws but not their annual budget laws, where the current amounts live — see `.scratch/es-autonomous-communities/`.
- **Percent literals are scalars in the store, and units outside the export vocabulary exist on purpose.** `paramdb.normalise_value` reads EUROMOD's `8.17%` as 0.0817 (the `/1` form the curated rates use) — before 8 Sep 2026 every NL/LT rate was stored as null and a correct proposal could only route `changed`. `KNOWN_UNITS` also accepts `currency/hour` (the SMIC is legislated per hour) and `ratio` (a multiplier above 1, e.g. NL `$bfa_mult2`, which the `/1` range check would reject). A bracket-schedule group has no text of its own in the export: `load_group_record` composes its label/description from the members, otherwise the frame query degenerates to `ES Tax schedule`. Re-run `ingest-params` + `curate-params` whenever the enriched export changes — FR/IE/LT ran for a month on the August export after the September one landed.
- **A derived value is arithmetic over quoted operands, checked like an extract.** When no extract states the value but the extracts state what it is computed from (NL `$bfa_mult2` = 347.83/286.45, a base plus a statutory increment, hourly rate × statutory hours), the proposer may set `derivation` + `operands`; `pipeline._check_derivation` verifies every operand's `supporting_extract` character-for-character in its cited chunk, that the extract states the operand's figure, that the expression uses only `+ - * /`, operand names and `DERIVATION_CONSTANTS` (1, 2, 4, 12, 13, 52, 100, 365, 366, 1000 — a remembered 151.67 is rejected), and that it evaluates to `value_scalar`. Each operand becomes its own `Reference`; the queue item carries the `derivation` summary. Never let a bare figure through unquoted — that is the hallucination channel the rule closes.
- **The scout's second leg looks the named source up in the corpus we hold.** Six of nine "corpus gap" refusals on the 2026-09-09 eval named acts already ingested (LIRPF art. 66, Wet IB 2001 art. 2.10, CSS D633-3, Finance Act 2024) that the framed query ranked below k. `retrieval.locate_named_units` resolves the proposal's `missing_sources` (act by title trigram similarity, article by number in `legal_units.citation`, else FTS inside the named act) and the `locate` step feeds those chunks to the retry, whether or not the scout ingested anything; they carry `method="located"` and the ScoutResult lists them under `located`. Web discovery needs Tavily; on HTTP 432 (plan quota spent — the whole 2026-09-09 eval ran that way) the scout says so once and skips further web calls.
- **A mistyped `citation_chunk_id` is resolved by its extract, never guessed.** The critique rewrites an id that is not among the retrieved chunks to the ONE retrieved chunk containing the `supporting_extract` character-for-character (`pipeline._resolve_chunk_by_extract`); two candidates keep the mechanical failure. The verbatim guarantee is unchanged — the extract is still checked against the chunk it now points to.
- **The parameter store is the source; parameter files are materialized from it.** `extracted_parameters/enriched/<CC>.enriched.json` is the EUROMOD-delivered ground truth and is read-only; everything else the DB needs arrives through the same `ingest-params` + `curate-params` path — the curation overlay `Nomoscope-agentic-workflow/pipeline/curation/<CC>.curation.yaml` carries what the export lacks or gets wrong (`temporal_basis`, `source_type`, and `unit`: the FR export labels all 146 `*rate*` parameters `currency`, IE 48 and LT 54 rate-like parameters likewise, all curated to `/1` — membership by value, every year's value a fraction — because the prompt's percent normalisation and the critique's range check key on it), re-applied after every `ingest-params` — `extracted_parameters/curated/<CC>.in_function.json` for parameters EUROMOD defines inside functions (the connector cannot export them; FR CDHR), and the export's own `groups` block for bracket schedules, reassembled into one record by `paramdb.load_group_record` (FR barème = `FR:tinkt_fr:tin_schedule`). Runtime inputs under `data/parameters/db|eval/` are derived artifacts, regenerated from the DB — never hand-edit one, and never let a golden case or a run depend on a hand-authored parameter file.
- **Golden readiness follows the value-bearing citation, and a golden citation list is any-of.** `citation_correct`/`retrieval_hit` accept any listed citation, so list every article a correct pipeline may cite (the finance act AND the consolidated code article; the by-reference rule AND the act stating the amount). An OpenFisca reference titled «(seuils avant revalorisation)» is the pre-indexation base article and does not make a case `corpus_available` (the year's amount is in a ministerial letter with no Legifrance id); a selection entry may set `corpus_available` explicitly in both builders. `score_item` completes a bare proposed scalar with the stored value's `#period` suffix before comparing, so `747#m` against a parameter EUROMOD holds as `8964#y` is a cross-period match, not a miss.
