...
Real LLM: `uv run euromod-workflow run-all --as-of 2025-06-01 --force --model anthropic/claude-sonnet-5`
...
- **In**: Activity 1 parameter records ([../Param_Schema/](../Param_Schema/)),
  pydantic-validated ([schema.py](pipeline/src/euromod_workflow/schema.py)).
- **Retrieval**: the legislation DB ([../RAG/db/schema.sql](../RAG/db/schema.sql));
...
([tracing.py](pipeline/src/euromod_workflow/tracing.py)):
...
```bash
cd agentic-workflow/pipeline && uv run --with pytest pytest   # mock extraction, diff, queue
