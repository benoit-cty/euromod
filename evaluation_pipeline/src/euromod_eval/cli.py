"""CLI: build the golden dataset, run evaluations, report KPIs.
  ...
  uv run euromod-eval init-db
  uv run euromod-eval build-dataset docs/fr_country_report_excerpt.md --country FR --as-of 2025-06-01
  uv run euromod-eval list-cases
...
def build_dataset_cmd(
    ...
    params_dir: Path = typer.Option(
        None, "--params-dir", help="Directory of Activity 1 parameter JSON files (default: agentic-workflow/data/parameters)"
    ),
    ...
    folder = params_dir or REPO_ROOT / "agentic-workflow" / "data" / "parameters"
