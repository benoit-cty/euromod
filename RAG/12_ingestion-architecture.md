...
`euromod-ingest fr instrument JORFTEXT000051168007 --with-consolidations --as-of-years 2025` executes the plan from the source analysis §3 as four worklist generations:
...
1. Where does the ingest package live — `RAG/ingest/` next to `db/`, or a top-level `ingest/`? (Affects the skill's import path.)
